...
func display(path string, v reflect.Value) {
	fmt.Println(strings.Join(sdisplay(path, v), "\n"))
}
...
func sdisplay(path string, v reflect.Value) []string {
	switch v.Kind() {
	case reflect.Invalid:
	...
	case reflect.Slice, reflect.Array:
		...
		for i := 0; i < v.Len(); i++ {
			results = append(results, sdisplay(fmt.Sprintf("%s[%d]", path, i), v.Index(i))...)
		}
		return results
	case reflect.Struct:
		results := make([]string, 0)
		for i := 0; i < v.NumField(); i++ {
			...
			results = append(results, sdisplay(fieldPath, v.Field(i))...)
		}
		return results
	case reflect.Map:
		results := make([]string, 0)
		for _, key := range v.MapKeys() {
			...
			results = append(results, sdisplay(fmt.Sprintf("%s[%s]", path,
				sdisplay("key", key)), v.MapIndex(key))...)
		}
		return results
	case reflect.Ptr:
		if v.IsNil() {
		...
		return sdisplay(fmt.Sprintf("(*%s)", path), v.Elem())
	case reflect.Interface:
		...
		results = append(results, sdisplay(path+".value", v.Elem())...)
		return results
